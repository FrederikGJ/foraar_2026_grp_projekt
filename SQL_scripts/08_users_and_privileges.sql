...
ON bilbase.* TO 'bilbase_app'@'%';

-- Admin user: full access
GRANT ALL PRIVILEGES
ON bilbase.* TO 'bilbase_admin'@'%';

-- Read-only user: can only read everything
GRANT SELECT
ON bilbase.* TO 'bilbase_readonly'@'%';

-- Restricted read user: can only read selected non-sensitive data
GRANT SELECT ON bilbase.brand TO 'bilbase_restricted'@'%';
GRANT SELECT ON bilbase.model TO 'bilbase_restricted'@'%';
GRANT SELECT ON bilbase.fuel_type TO 'bilbase_restricted'@'%';
GRANT SELECT ON bilbase.region TO 'bilbase_restricted'@'%';

-- If your view exists, allow restricted user to read active listings too
GRANT SELECT ON bilbase.active_listings TO 'bilbase_restricted'@'%';
